...
class CameraWithImagesApp(App):
    ...
    def __init__(self):
        ...
        self.led_colour = 1     # Red = 0, Blue = 1
        self.encoding = 2       # CRC = 0, Parity = 1, Hamming = 2
    ...
    def build(self):
        ...
        led_button_layout = BoxLayout(orientation='horizontal', size_hint=(1, None), height=50)
        ...
        led_button_layout.add_widget(red_button)
        led_button_layout.add_widget(blue_button)
        ...
        encoding_button_layout = BoxLayout(orientation='horizontal', size_hint=(1, None), height=50)
        crc_button = Button(text='CRC', size_hint=(0.33, 1))
        crc_button.bind(on_press=self.set_crc_encoding)
        parity_button = Button(text='Parity', size_hint=(0.33, 1))
        parity_button.bind(on_press=self.set_parity_encoding)
        hamming_button = Button(text='Hamming', size_hint=(0.33, 1))
        hamming_button.bind(on_press=self.set_hamming_encoding)
        encoding_button_layout.add_widget(crc_button)
        encoding_button_layout.add_widget(parity_button)
        encoding_button_layout.add_widget(hamming_button)

        # Add the Camera and Camera Image widgets to the layout

        layout.add_widget(led_button_layout)
        layout.add_widget(self.camera_frame)
        layout.add_widget(self.progress_image)
        layout.add_widget(encoding_button_layout)
    ...
    def set_crc_encoding(self, instance):
        self.encoding = 0

    def set_parity_encoding(self, instance):
        self.encoding = 1

    def set_hamming_encoding(self, instance):
        self.encoding = 2
    ...
    def on_request_success(self, request, result):
        ...
        if hall and building:
            ...
            if self.correct_room:
                ...
                customMessage = "No custom message for you!"
            ...
            popup_msg = f"Building and Hall not found in the HTML code"
            self.show_popup(popup_msg)
    ...
    def display_room_info(self):
        ...
        if 0 < x < 213:
            UrlRequest(url, on_success=self.on_request_success)
        else:
            popup_msg = "Invalid Space Number"
            self.show_popup(popup_msg)
    ...
    def receive_message(self, frame):
        ...
        # === BLUE MASK ===
        # DEFAULT:
        # lower_blue = np.array([100, 90, 235], np.uint8)
        ...
        if self.bool_read_message:
            if self.encoding == 0:
                ...
                if len(self.decoded_bitstream) < 12:
                    if self.skip == 0:
                        self.decode_message()
                else:
                    self.bool_message_received = True
        ...
        if self.bool_message_received:
            ...
            if self.encoding == 0:
                ...
                decoded_msg = hamming_codec.decode(int(self.received_message, 2), 12, hamming_codec.ParityLocation.LSB,
                                                   n_parity_bits=4)
                self.received_message = decoded_msg
                ...
                # msg_string = decode_message(message)
                if self.debug:
                    print(f"Message received: {decoded_msg}")
                self.display_room_info()


            if self.received_message == '11001101':
                self.correct_msg += 1
        ...
        if self.debug:
            ...
            if self.msg_count > 0:
                print(f"CORRECT %: {self.correct_msg / self.msg_count * 100}")
            print("====================================================")
