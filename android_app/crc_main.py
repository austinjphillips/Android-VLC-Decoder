...
class AndroidCamera(Camera):
    ...
    def receive_message(self, frame):
        ...
        # === RED MASK ===
        # lower_red = np.array([165, 0, 0], np.uint8)
        # upper_red = np.array([255, 60, 60], np.uint8)
        # red_mask = cv2.inRange(frame, lower_red, upper_red)

        # === BLUE MASK ===
        hsv_image = cv2.cvtColor(frame, cv2.COLOR_RGB2HSV)
        lower_blue = np.array([95, 50, 245], np.uint8)
        ...
        # thresh = cv2.erode(red_mask, None, iterations=1)
        thresh = cv2.dilate(blue_mask, None, iterations=10)
        ...
        if self.sync_bits in self.raw_bitstream:
