...
int encodeHamming( byte data )
{
  Serial.println("");
  byte dataByte1 = 0b01011011 & data;
  bool parityBit1 = calculateParityBit(dataByte1);
  Serial.println(parityBit1);

  byte dataByte2 = 0b01101101 & data;
  bool parityBit2 = calculateParityBit(dataByte2);
  Serial.println(parityBit2);


  byte dataByte3 = 0b10001110 & data;
  bool parityBit3 = calculateParityBit(dataByte3);
  Serial.println(parityBit3);


  byte dataByte4 = 0b11110000 & data;
  bool parityBit4 = calculateParityBit(dataByte4);
  Serial.println(parityBit4);


  int encodedData = data << 4 | parityBit4 << 3 | parityBit3 << 2 | parityBit2 << 1 | parityBit1;
  Serial.println(encodedData, BIN);
  Serial.println("");
  return encodedData;
}
...
void buildFrame( int payLoad )
{
  ...
  byte payload = (int) dueFlashStorage.read( 1 );

  int encodedData = encodeHamming(payload);
  Serial.println("Encoded Data: ");
  Serial.println(encodedData, BIN);

  bool parity = calculateParityBit(payload);
  ...
  {
    Serial.print("bit: ");
    Serial.println(bitRead(encodedData, i-1));
    sendBit(bitRead(encodedData, i-1));
  }
  // sendBit(parity);
